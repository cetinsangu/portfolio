    ...
    <header className="w-full px-32 py-8 font-medium flex items-center justify-between dark:text-light relative z-10 lg:px-16 md:px-12 sm:px-8">
      ...
      {isOpen ? (
        <motion.div
          ...
          className="min-w-[70vw] flex flex-col justify-between z-30 items-center fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-dark/90 dark:bg-light/75 rounded-lg backdrop-blur-md py-32"
        >
          ...
          <nav className="flex items-center justify-between flex-wrap mt-2">
            <motion.a
              ...
              target={"_blank"}
              whileHover={{ y: -2 }}
              className="w-8 mx-2 bg-light rounded-full dark:bg-dark sm:mx-1"
              whileTap={{ scale: 0.9 }}
            >
              <XIcon className={mode === "dark" ? "fill-white" : "fill-dark"} />
            </motion.a>
            ...
            <button
              className={`ml-12 w-12 flex items-center justify-center rounded-full sm:mx-1 p-1 ${
                mode === "light" ? "bg-dark text-light" : "bg-light text-dark"
              }`}
              onClick={() => setMode(mode === "light" ? "dark" : "light")}
            >
